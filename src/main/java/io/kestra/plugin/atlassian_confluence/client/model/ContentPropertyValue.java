...
public class ContentPropertyValue extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(ContentPropertyValue.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!ContentPropertyValue.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'ContentPropertyValue' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<ContentPropertyValue>() {
                @Override
                public void write(JsonWriter out, ContentPropertyValue value) throws IOException {
                ...
                @Override
                public ContentPropertyValue read(JsonReader in) throws IOException {
                    ...
                    try {
                        ...
                        // deserialization failed, continue
                        errorMessages.add(String.format(Locale.ROOT, "Deserialization for Map<String, Object> failed with `%s`.", e.getMessage()));
                    ...
                    if (match == 1) {
                        ContentPropertyValue ret = new ContentPropertyValue();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format(Locale.ROOT, "Failed deserialization for ContentPropertyValue: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public ContentPropertyValue(Object o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return ContentPropertyValue.schemas;
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to ContentPropertyValue
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format(Locale.ROOT, "The JSON string is invalid for ContentPropertyValue with oneOf schemas: Boolean, List<String>, Map<String, Object>, String. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of ContentPropertyValue given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of ContentPropertyValue
     * @throws IOException if the JSON string is invalid with respect to ContentPropertyValue
     */
    public static ContentPropertyValue fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, ContentPropertyValue.class);
    }

    /**
     * Convert an instance of ContentPropertyValue to an JSON string
     *
     * @return JSON string
     */
