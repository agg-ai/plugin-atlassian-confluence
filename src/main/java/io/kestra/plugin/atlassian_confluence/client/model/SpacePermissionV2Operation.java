...
public class SpacePermissionV2Operation {
  ...
  public SpacePermissionV2Operation key(@javax.annotation.Nonnull KeyEnum key) {
    this.key = key;
    return this;
  }
  ...
  public SpacePermissionV2Operation target(@javax.annotation.Nonnull TargetEnum target) {
    this.target = target;
    return this;
  }
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("key", "target"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("key", "target"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SpacePermissionV2Operation.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in SpacePermissionV2Operation is not found in the empty JSON string", SpacePermissionV2Operation.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SpacePermissionV2Operation.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `SpacePermissionV2Operation` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
