...
public class MessageArgsInner extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(MessageArgsInner.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!MessageArgsInner.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'MessageArgsInner' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<MessageArgsInner>() {
                @Override
                public void write(JsonWriter out, MessageArgsInner value) throws IOException {
                ...
                @Override
                public MessageArgsInner read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        MessageArgsInner ret = new MessageArgsInner();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format(Locale.ROOT, "Failed deserialization for MessageArgsInner: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public MessageArgsInner(Object o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return MessageArgsInner.schemas;
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to MessageArgsInner
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format(Locale.ROOT, "The JSON string is invalid for MessageArgsInner with oneOf schemas: Map<String, Object>, String. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of MessageArgsInner given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of MessageArgsInner
     * @throws IOException if the JSON string is invalid with respect to MessageArgsInner
     */
    public static MessageArgsInner fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, MessageArgsInner.class);
    }

    /**
     * Convert an instance of MessageArgsInner to an JSON string
     *
     * @return JSON string
     */
